...
// should appear in the public changelog. The trailer prefix picks the SemVer
// bump (semver.org):
//
//   更新: 新增委托历史模块       → minor bump（新功能）
//   修复: 修复手机端注释不显示   → patch bump（问题修复）
//   重大: 站点导航与路由重构     → major bump（不兼容变更，minor/patch 归零）
...
// the generated file, and amend it into the same commit. Commits without either
// trailer are ignored, so routine refactors stay out of the changelog.
//
// The changelog is visitor-facing, so work on the private analytics dashboard
// (/analytics) is deliberately kept out of it: don't add a trailer to those
...
const OVERRIDES = new Map([
  ["8bb369f", { zh: "仓位计算器新增反推风险计算模块", en: "Added a reverse risk calculation module to the position calculator" }],
  ["bd1a172", { zh: "复盘模式新增进场标签及改进点等", en: "Added entry tags and improvement points to review mode" }],
  ["56a4c18", { zh: "复盘模式新增进场标签及改进点等", en: "Added entry tags and improvement points to review mode" }],
  ["5c942dd", { zh: "复盘模式新增进场标签及改进点等", en: "Added entry tags and improvement points to review mode" }],
  ...
  ["16de62e", { zh: "修正统一账户可用余额计算逻辑", en: "Fixed unified account available balance calculation logic" }],
...
function readTrailerCommits() {
  ...
  for (const record of raw.split("\x1e")) {
    ...
    // 修复:/重大: only count in the body — otherwise routine "修复: ..."
    // subjects (this repo's commit style) would flood the public changelog.
    // On conflicts the highest severity wins: 重大 > 更新 > 修复.
    const bodyAfterSubject = body.split("\n").slice(1).join("\n");
    const breaking = matchTrailer(bodyAfterSubject, /^重大[:：]\s*(.+)$/m);
    const feature = matchTrailer(body, /^更新[:：]\s*(.+)$/m);
    const fix = matchTrailer(bodyAfterSubject, /^修复[:：]\s*(.+)$/m);
    const zh = breaking ?? feature ?? fix;
    if (!zh) continue;
    const kind = KIND_OVERRIDES.get(shortSha) ?? (breaking ? "major" : feature ? "minor" : "patch");
...
function build() {
  const entries = [...SEED];
  let version = SEED[SEED.length - 1].version;
  ...
  for (const commit of readTrailerCommits()) {
    ...
    version = bumpVersion(version, commit.kind);
