...
const USER_ADDRESS =
  process.env.HYPERLIQUID_USER_ADDRESS || process.env.HYPERLIQUID_ADDRESS || "";
const MANUAL_INITIAL_CAPITAL_USDC =
  process.env.HYPERLIQUID_INITIAL_CAPITAL_USDC || "";
...
function assertAddress() {
  ...
  if (!/^0x[a-fA-F0-9]{40}$/.test(address)) {
    throw new Error(
      "Hyperliquid account address is not configured. Please set HYPERLIQUID_USER_ADDRESS=0x..."
    );
  }
  return address;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    response.on("end", () => {
      try {
        ...
        reject(
          new Error(
            `Hyperliquid returned a non-JSON response: ${text.slice(0, 120)}`
          )
        );
      }
    });
...
function requestJson<T>(
  ...
  if (!proxy) {
    return new Promise((resolve, reject) => {
      ...
      request.on("timeout", () =>
        request.destroy(new Error("Hyperliquid request timed out"))
      );
  ...
  return new Promise((resolve, reject) => {
    ...
    connect.on("connect", (response, socket) => {
      if (response.statusCode !== 200) {
        ...
        reject(
          new Error(
            `Hyperliquid proxy connection failed: ${response.statusCode}`
          )
        );
        return;
      }
    ...
    connect.on("timeout", () =>
      connect.destroy(new Error("Hyperliquid proxy connection timed out"))
    );
...
function logHyperliquidPositions(
  dex: string,
...
function hasOpenPerpExposure(state: HyperliquidClearinghouseState) {
  ...
  const summary = state.marginSummary ?? state.crossMarginSummary ?? {};
  return (
    positions.some(({ position }) => Math.abs(toNumber(position.szi)) > 0) ||
    Math.abs(toNumber(summary.totalNtlPos)) > 0
  );
}
...
export async function getHyperliquidSpotState() {
  const user = assertAddress();
  return callInfo<HyperliquidSpotClearinghouseState>({
    type: "spotClearinghouseState",
    user,
  });
}

export function getHyperliquidSpotEquityUsdc(
  spotState: HyperliquidSpotClearinghouseState
) {
  const balances = spotState.balances ?? [];
  return balances.reduce((sum, balance) => {
    const coin = balance.coin.toUpperCase();
    if (coin === "USDC" || coin === "USDC.E")
...
export function getHyperliquidSpotUsdcBalance(
  spotState: HyperliquidSpotClearinghouseState
) {
...
export async function getHyperliquidFills(
  startTime?: number,
  endTime?: number
) {
  const user = assertAddress();
...
export async function getHyperliquidOpenOrders() {
  ...
  const orders = results.flatMap(result => {
    if (result.status === "fulfilled") {
      return result.value.orders.map(order => ({
        ...order,
...
export async function getHyperliquidLedgerUpdates(
  startTime = 0,
  endTime = Date.now()
) {
  const user = assertAddress();
...
export function getLatestHyperliquidPortfolioEquity(
...
export function getInitialHyperliquidPortfolioEquity(
  portfolio: HyperliquidPortfolio
) {
  const preferred =
    findPortfolioWindow(portfolio, "allTime") ??
...
export function getHyperliquidPortfolioEquitySummary(
  portfolio: HyperliquidPortfolio
) {
  return {
...
export function getHyperliquidMaxDrawdown(portfolio: HyperliquidPortfolio) {
  ...
  return {
    ...
    maxDrawdownPct:
      maxDrawdownPeak > 0 ? -(maxDrawdown / maxDrawdownPeak) * 100 : null,
  };
}
...
export function getHyperliquidPerformanceStats(
  portfolio: HyperliquidPortfolio
) {
  const windowData =
    findPortfolioWindow(portfolio, "allTime") ??
  ...
  const runningDays = Math.max(
    1,
    Math.ceil((Date.now() - firstTime) / (24 * 60 * 60 * 1000))
  );
  ...
  const sharpeRatio =
    volatility > 0 ? (meanReturn / volatility) * Math.sqrt(365) : null;
...
function calculateRoundTripTradeMetrics(fills: HyperliquidFill[]) {
  ...
  const sortedFills = fills.slice().sort((a, b) => a.time - b.time);
  ...
  for (const fill of sortedFills) {
    ...
    if (endsFlat || flipsSide) {
      ...
      if (closingTrade) {
        ...
        const holdingHours =
          (fill.time - closingTrade.openedAt) / (60 * 60 * 1000);
  ...
  const grossWin = completedPnls.reduce(
    (sum, pnl) => sum + Math.max(0, pnl),
    0
  );
  const grossLoss = Math.abs(
    completedPnls.reduce((sum, pnl) => sum + Math.min(0, pnl), 0)
  );
  const avgWin = winningTrades > 0 ? grossWin / winningTrades : 0;
  const avgLoss = losingTrades > 0 ? grossLoss / losingTrades : 0;
  const averageHoldingHours =
    completedHoldingHours.length > 0
      ? completedHoldingHours.reduce((sum, hours) => sum + hours, 0) /
        completedHoldingHours.length
      : null;
  ...
  return {
    ...
    breakevenTrades,
    winRate:
      completedPnls.length > 0
        ? (winningTrades / completedPnls.length) * 100
        : null,
...
export async function getHyperliquidPortfolioSnapshots(params: {
  ...
  const startMs = params.startDate
    ? new Date(`${params.startDate}T00:00:00Z`).getTime()
    : 0;
  const endMs = params.endDate
    ? new Date(`${params.endDate}T23:59:59Z`).getTime()
    : Number.MAX_SAFE_INTEGER;
  const pnlHistory = windowData.pnlHistory ?? [];
  const accountValueHistory = windowData.accountValueHistory ?? [];
  const baseEquity =
    accountValueHistory.length > 0 ? toNumber(accountValueHistory[0][1]) : 0;
  ...
  const lastTime =
    filteredHistory[filteredHistory.length - 1]?.[0] ??
    Math.min(endMs, Date.now());
  ...
  return filteredHistory.map(([time, equity]) => {
    const totalPnl =
      nearestHistoryValue(pnlHistory, time) ??
      String(toNumber(equity) - baseEquity);
    return {
      currency: "USDC",
...
export async function getHyperliquidPositions() {
  ...
  return states.flatMap(({ dex, state }) => {
    ...
    return (state.assetPositions ?? []).map(({ position }) => {
      ...
      const mark =
        Math.abs(size) > 0 && toNumber(position.positionValue) > 0
...
export async function getHyperliquidAccountOverview() {
  ...
  const perpEquityUsdc = summaries.reduce(
    (sum, summary) => sum + toNumber(summary.accountValue),
    0
  );
  const spotEquityUsdc = getHyperliquidSpotEquityUsdc(spotState);
  const spotUsdcBalance = getHyperliquidSpotUsdcBalance(spotState);
  const fallbackEquityUsdc =
    portfolioEquity.latest && portfolioEquity.latest > 0
      ? portfolioEquity.latest
      : perpEquityUsdc;
  const totalEquityUsdc =
    spotEquityUsdc > 0 ? spotEquityUsdc : fallbackEquityUsdc;
  const totalMarginUsed = summaries.reduce(
    (sum, summary) => sum + toNumber(summary.totalMarginUsed),
    0
  );
  const totalNtlPos = summaries.reduce(
    (sum, summary) => sum + toNumber(summary.totalNtlPos),
    0
  );
  const withdrawable = activePerpStates.reduce(
    (sum, { state }) => sum + toNumber(state.withdrawable),
    0
  );
  const positions = activePerpStates.flatMap(
    ({ state }) => state.assetPositions ?? []
  );
  ...
  const totalPnlUsdc =
    initialEquityUsdc && initialEquityUsdc > 0
      ? totalEquityUsdc - initialEquityUsdc
      : null;
  const totalPnlPct =
    totalPnlUsdc != null && initialEquityUsdc && initialEquityUsdc > 0
  ...
  return {
    ...
    annualizedReturnPct:
      accountAnnualizedReturnPct ?? performance.annualizedReturnPct,
...
export async function getHyperliquidTradeHistory(params: {
  ...
  const mapped = fills
    ...
    .map(fill => {
      ...
      return {
        execId: fill.hash
          ? `${fill.hash}-${fill.oid}-${fill.time}`
          : `${fill.coin}-${fill.oid}-${fill.time}`,
