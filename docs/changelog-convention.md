...
| 尾注 | 含义 | 版本效果 |
|---|---|---|
| `更新: <一句话>` | 新功能 | minor +1（如 1.44.1 → 1.45.0） |
| `修复: <一句话>` | 问题修复 | patch +1（如 1.44.1 → 1.44.2） |
| `重大: <一句话>` | 不兼容变更（换代级重构） | major +1，其余归零（如 1.44.1 → 2.0.0） |
...
- `更新:` 兼容匹配整段提交信息（早期有单行 "更新: ..." 当标题的历史提交）；`修复:`、`重大:` **只匹配正文**，防止 "修复: ..." 标题的日常提交误入日志。
- 一条提交带多个尾注时按最高级别处理：`重大:` > `更新:` > `修复:`。
...
- 双尾注约定（2026-07-19）生效前用错 bump 类型的历史条目，在脚本的 `KIND_OVERRIDES` 表里按 SHA 修正（如 `5d08245` 修复误发为 minor，已改记为 patch）；新规则不溯及，其余历史版本保持原样。
