...
export default function TradeHistory() {
  ...
  const { data, isLoading, isFetching, refetch, error } =
    trpc.hyperliquid.tradeHistory.useQuery(
      {
        category,
  ...
  const totalFees = filtered.reduce((sum, trade) => {
    return (
      sum +
      (trade.feeDetail ?? []).reduce(
        (feeSum, item) => feeSum + Math.abs(num(item.fee)),
        0
      )
    );
  }, 0);
      ...
      {filtered.length > 0 && (
        ...
        >
          <div>
            ...
            <div className="num-display" style={{ fontSize: "0.78rem" }}>
              {filtered.length}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div
              ...
              style={{ fontSize: "0.78rem", color: pnlColor(totalPnl) }}
            >
              {signed(totalPnl, 2)}
            </div>
          </div>
                ...
                {pageTrades.map((trade, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {fmtTime(trade.createdTime)}
                      </td>
                      <td
                        className="py-2 pr-4 text-foreground font-medium"
                        style={{ fontSize: "0.72rem" }}
                      >
                        {trade.symbol}
                      </td>
                      ...
                      <td
                        ...
                        style={{ fontSize: "0.72rem" }}
                      >
                        {fmt(trade.execPrice, 2)}
                      </td>
                      <td
                        className="py-2 pr-4 num-display"
                        style={{ fontSize: "0.72rem" }}
                      >
                        {fmt(trade.execValue, 2)}
                      </td>
                      <td
                        className="py-2 pr-4 num-display"
                        style={{
                      ...
                      >
                        {fee ? `${fmt(fee.fee, 2)} ${fee.feeCoin}` : "—"}
                      </td>
                      <td
                        className="py-2 pr-4 num-display"
                        style={{ fontSize: "0.72rem", color: pnlColor(pnl) }}
                      >
                        {pnl !== 0 ? signed(pnl, 2) : "—"}
                      </td>
                    </tr>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span>
                      {t("价格", "Price")}: {fmt(trade.execPrice, 2)}
                    </span>
                    <span>
                      {t("数量", "Qty")}: {fmt(trade.execQty, 2)}
                    </span>
                    {num(trade.execPnl) !== 0 && (
                      <span style={{ color: pnlColor(trade.execPnl) }}>
                        PnL: {signed(trade.execPnl, 2)}
                      </span>
                    )}
                  </div>
                </div>
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={page === 0}
                className="px-2 py-0.5 rounded border border-border/30 text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors"
              >
                {t("上一页", "Prev")}
              </button>
              <button
