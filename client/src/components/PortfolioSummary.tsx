...
function PnlValue({ value }: { value: number }) {
  const cls =
    value > 0 ? "text-profit" : value < 0 ? "text-loss" : "text-neutral";
  return (
...
export default function PortfolioSummary() {
  ...
  const { data, isLoading, error, refetch, isFetching } =
    trpc.deribit.accountSummaries.useQuery(undefined, {
  ...
  return (
    <div className="glass-card px-8 py-7 fade-in">
        ...
        <div key={s.currency} className="mb-8 last:mb-0">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-3">
            ...
            <StatBox
              label={tr.balance}
              value={
                <span className="text-foreground/90">{fmt(s.balance)}</span>
              }
              sub={s.currency}
            />
            <StatBox
            ...
            <StatBox
              label={tr.deltaTotal}
              value={
                <span className="text-neutral">{fmt(s.delta_total, 2)}</span>
              }
            />
            <StatBox
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <StatBox
              label={tr.realizedPnl}
              value={<PnlValue value={s.realized_pl} />}
              sub={s.currency}
            />
            <StatBox
              label={tr.futuresPnl}
              value={<PnlValue value={s.futures_pl} />}
              sub={s.currency}
            />
            <StatBox
              label={tr.optionsPnl}
              value={<PnlValue value={s.options_pl} />}
              sub={s.currency}
            />
            <StatBox
              label={tr.optionsValue}
              value={
                <span className="text-foreground/80">
                  {fmt(s.options_value)}
                </span>
      ...
      {btc && eth && (
        ...
        >
          <div
            className="text-muted-foreground tracking-widest uppercase mb-3"
            style={{ fontSize: "0.65rem" }}
          >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <StatBox
              label={tr.btcBalance}
              value={
                <span className="text-foreground">{fmt(btc.balance, 2)}</span>
              }
              sub="BTC"
            />
            <StatBox
              label={tr.ethBalance}
              value={
                <span className="text-foreground">{fmt(eth.balance, 2)}</span>
              }
              sub="ETH"
            />
            <StatBox
              label="BTC Delta"
              value={
                <span className="text-neutral">{fmt(btc.delta_total, 2)}</span>
              }
            />
            <StatBox
              label="ETH Delta"
              value={
                <span className="text-neutral">{fmt(eth.delta_total, 2)}</span>
              }
            />
          </div>
        </div>
